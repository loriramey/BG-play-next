...
def display_interactive_charts():
    ...
    dataset_option = st.selectbox("Choose Dataset:", options=["Top 300 Games", "Full Dataset"])
    ...
    # Convert key columns to numeric, handling errors as NaN
    df["bayesaverage"] = pd.to_numeric(df["bayesaverage"], errors="coerce")
    df["playingtime"] = pd.to_numeric(df["playingtime"], errors="coerce")
    df["averageweight"] = pd.to_numeric(df["averageweight"], errors="coerce")
    df["BGGrank"] = pd.to_numeric(df["BGGrank"], errors="coerce")
    ...
    if chart_option == "Bayesian Rating vs Playtime":
        x_col = "playingtime"
        x_label = "Playtime (in minutes)"
    elif chart_option == "Bayesian Rating vs BGG Rank":
        x_col = "BGGrank"
        x_label = "BGG Rank"
    elif chart_option == "Bayesian Rating vs Game Weight":
        x_col = "averageweight"
